...
class Blade {
	...
	{
		preg_match_all('/\{\{.*?\}\}/', $value, $blade_templates);

		$replaceables = array();
		foreach ($blade_templates as $key => $val)
		{
			foreach ($val as $key1 => $val1)
			{
				preg_match_all('/[a-z].*[a-z]/', $val1, $template_regex);
				$replaceables[$val1] = $template_regex[0][0];
			}
		}

		$to_replace = array();
		foreach ($replaceables as $key => $val)
		{
			if(strrpos($val, '.'))
			{
